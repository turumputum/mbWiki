...
> Описание модуля **button_led** для прошивок до перехода на
> [[Платформа moduleBox/Конституция|конституцию]] (топики `button_<slot>` / `led_<slot>` без `event`/`action`, опция `topic`).
> Актуальное описание — [[Программные модули/Кнопки с подсветкой/button_led_sw|button_led]].
